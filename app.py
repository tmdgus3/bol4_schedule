...
if can_edit:
    ...
    if edit_index is not None and edit_index in df.index:
        ...
        try:
            default_time = dtime.fromisoformat(row["Time"])
        except:
    ...
    with st.form("form_add"):
        ...
        st.markdown("**주소 또는 장소 검색**")
        address_input = st.text_input("주소를 입력하세요", default_location)
        geocode_button = st.form_submit_button("주소 검색")
        confirmed_location = address_input
        if geocode_button and address_input:
            location = geolocator.geocode(address_input)
            if location:
                st.success(f"📍 주소 인식됨: {location.address}")
                confirmed_location = location.address
            else:
        ...
        if submitted:
            new_data = pd.DataFrame([[date, time.strftime("%H:%M"), title, memo, confirmed_location]], columns=df.columns)
...
if not df.empty:
    ...
    m.fit_bounds([[33.0, 124.5], [38.7, 131.2]])  # 대한민국 전체 범위
